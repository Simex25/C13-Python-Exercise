from Simi.practice.oop import Dog, Car, GoldenRetriever, Square

if __name__ == "__main__":
    my_dog = Dog("Philo",4)

    my_dog.description()
    print(my_dog.sound("Woof Woff"))
    my_dog.coat_colour("blue")

    my_car = Car("Blue", 20000)
    my_car2 = Car("Red",0)
    print(f"The {my_car.colour} car has {my_car.mileage} miles")
    print(f"The {my_car2.colour} car has driven {my_car2.drive(100)} miles")
    new_dog = GoldenRetriever("philo",7)
    print(f"{new_dog.name}, sound is {new_dog.speak()}")
